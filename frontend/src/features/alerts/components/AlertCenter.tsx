        ...
        <div className="space-y-6">
            <div className="bento-card p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-black text-slate-900 m-0">Live & Persistent Alerts</h2>
                        <p className="text-xs text-slate-500 font-medium m-0">Real-time alerts broadcast via Socket.IO and stored in MongoDB</p>
                    </div>
                </div>
                <div className="text-xs text-slate-700 bg-slate-100 px-3.5 py-2 rounded-xl border border-slate-200 font-semibold">
                    Active Unresolved Alerts: <span className="text-rose-600 font-extrabold">{alerts.filter(a => a && a.status === 'active').length}</span>
                </div>
            </div>
                            ...
                            >
                                <div className="flex items-start gap-3.5">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className="text-sm font-bold text-slate-900">{animalName}</span>
                                            <span className={`text-[10px] uppercase font-extrabold px-2 py-0.5 rounded-full ${isCritical ? 'bg-rose-100 text-rose-700 border border-rose-200' : 'bg-amber-100 text-amber-700 border border-amber-200'}`}>
                                                {alert.type || 'ALERT'}
                                            </span>
                                            <span className="text-xs text-slate-400 font-medium">• {new Date(alert.createdAt || Date.now()).toLocaleTimeString()}</span>
                                        </div>
                                        <p className="text-xs text-slate-700 font-medium m-0 leading-relaxed">{alert.message}</p>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-xs font-bold border border-emerald-200 transition"
                                    >
